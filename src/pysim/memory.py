...
class SwarmMemory(object):
    ...
    def add(self, state, action, action_logprobs, reward, done, next_obs=None, intrinsic_reward=None, locked_mask=None, duration=None):
        ...
        for i in range(self.num_agents):
            ...
            lock_ = locked_mask[i] if locked_mask is not None else None
            self.memory[i].add(
...
class Memory(object):
    ...
    def __init__(self, action_dim=2, max_size=int(1e5), use_intrinsic_reward=False, use_next_obs=False, use_locked_mask=False, use_duration=False):
        ...
        # locked_mask: per-step per-drone bool flag indicating drones whose actions
        # were structurally overridden (goal-commitment lock). Stored as float32 0/1
        # to share the pinned-buffer machinery; cast to bool at to_tensor() time.
        # Shape (max_size, num_drones) — pulled from action_dim[0] when action_dim is
        # a tuple (planner: (num_drones, 2)). Falls back to scalar shape otherwise.
        self.use_locked_mask = use_locked_mask
        if self.use_locked_mask:
            num_drones = action_dim[0] if isinstance(action_dim, (tuple, list)) and len(action_dim) >= 1 else 1
    ...
    def state_dict(self) -> Dict[str, Any]:
        ...
        data = {
            ...
            "use_locked_mask": self.use_locked_mask,
            "locked_mask": (
    ...
    def load_state_dict(self, state: Dict[str, Any]):
        ...
        if self.use_locked_mask:
            num_drones = self.action_dim[0] if isinstance(self.action_dim, (tuple, list)) and len(self.action_dim) >= 1 else 1
            self.locked_mask = self._alloc_buffer((self.max_size, num_drones))
        else:
            self.locked_mask = None
    ...
    def to_tensor(self):
        ...
        # Only add locked_mask if it is available. Cast to bool here — buffer is float
        # to share the pinned-allocator path, but consumers want bool semantics.
        if self.use_locked_mask:
    ...
    def gather(self, idx, *, as_tensors=True):
        ...
        if self.use_locked_mask and self.locked_mask is not None:
            out["locked_mask"] = torch.as_tensor(
    ...
    def change_horizon(self, new_horizon):
        ...
        if self.use_next_obs:
            ...
            self.next_obs = None
        if self.use_locked_mask:
            num_drones = self.locked_mask.shape[1] if self.locked_mask is not None else 1
            self.locked_mask = self._alloc_buffer((self.max_size, num_drones))
        else:
            self.locked_mask = None
        if self.use_duration:
