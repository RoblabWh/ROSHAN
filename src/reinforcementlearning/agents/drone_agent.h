...
//public:
//    explicit DroneAgent(const std::shared_ptr<GridMap>& grid_map, std::string agent_type, FireModelParameters &parameters, int id, int timesteps);
...
//    std::vector<bool> IsTerminal(bool eval_mode, const std::shared_ptr<GridMap>& grid_map, int total_env_steps);
